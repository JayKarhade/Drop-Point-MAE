...
# Pretrain model and also drop positions
class MaskTransformer(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        self.drop_mask_ratio = config.transformer_config.position_mask_ratio
        self.drop_mask_token = nn.Parameter(torch.zeros(1, 1, self.trans_dim))
    ...
    def forward(self, neighborhood, center, noaug = False):
        ...
        #generate dropped position indices and replace the mask with learnable mask token
        bool_drop_pos = torch.zeros((masked_center.shape[0],masked_center.shape[1])).to(center.device)
        for i in range(center.shape[0]): #loop over batches
            #get the bool drop masks array and convert to torch bool
            cur_bool_drop_mask = np.ones((pos.shape[1]))
            cur_bool_drop_mask[:int(self.drop_mask_ratio*pos.shape[1])] = 0
            np.random.shuffle(cur_bool_drop_mask)
            cur_bool_drop_mask = torch.from_numpy(cur_bool_drop_mask).to(torch.bool)
            replace_mask = ~cur_bool_drop_mask
            pos[i,replace_mask] = self.drop_mask_token
            bool_drop_pos[i] = replace_mask
        bool_drop_pos = bool_drop_pos.to(torch.bool)
        # transformer
        x_vis = self.blocks(x_vis, pos)
        x_vis = self.norm(x_vis)

        return x_vis,masked_center,bool_masked_pos,bool_drop_pos


@MODELS.register_module()
class Drop_Point_MAE(nn.Module):
    def __init__(self, config):
        super().__init__()
        print_log(f'[Drop_Point_MAE] ', logger ='Drop_Point_MAE')
        self.config = config
        self.trans_dim = config.transformer_config.trans_dim
        ...
        print_log(f'[Drop_Point_MAE] divide point cloud into G{self.num_group} x S{self.group_size} points ...', logger ='Drop_Point_MAE')
    ...
    def forward(self, pts, vis = False, **kwargs):
        ...
        x_vis,vis_centers,mask,drop_pos_mask = self.MAE_encoder(neighborhood,center) #returns x_vis encoded tokens, mask corresponding to the x_vis tokens, and drop_pos_mask of dropped PEs
        B,_,C = x_vis.shape
        ...
        loss1 = torch.mean(torch.sqrt((pred_pos[:,:,0] - gt_centers[:,:,0])**2 + (pred_pos[:,:,1] - gt_centers[:,:,1])**2 + (pred_pos[:,:,2] - gt_centers[:,:,2])**2))
