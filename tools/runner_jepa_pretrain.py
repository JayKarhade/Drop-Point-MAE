...
def jepa_run_net(args, config, train_writer=None, val_writer=None):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            # import copy
            # student_model = copy.deepcopy(base_model.module.target_encoder.eval())
            # orig_param_list = []

            # for student_param in student_model.parameters():
            #     orig_param_list.append(student_param)
            ...
            if num_iter == config.step_per_update:
                ...
                # for student_param in student_model.parameters():
                #     post_optim_list.append(student_param)

                # tot_sum = 0
                # for idx in range(len(post_optim_list)):
                #     tot_sum = tot_sum + torch.sum(post_optim_list[idx] - orig_param_list[idx])
                # print(tot_sum)
                ...
                with torch.no_grad():
                    ...
                    #Debugging to see if any change
                    # student_model = copy.deepcopy(base_model.module.target_encoder.eval())
                    # post_momentum_list = []
                    # for student_param in student_model.parameters():
                    #     post_momentum_list.append(student_param)

                    # tot_sum = 0
                    # for idx in range(len(post_momentum_list)):
                    #     tot_sum = tot_sum + torch.sum(post_momentum_list[idx] - orig_param_list[idx])
                    # print(tot_sum)
